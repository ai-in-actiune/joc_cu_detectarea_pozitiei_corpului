class Fighter:
    def __init__(self, life=100, max_power=40, lpa_coefficient=0.10, hpa_coefficient=0.18, shield_lpa_coefficient=0.05, shield_hpa_coefficient=0.20):
        self.life = life
        self.maxLPA = lpa_coefficient * self.life
        self.maxHPA = hpa_coefficient * self.life
        self.shieldLPA = shield_lpa_coefficient * self.maxHPA
        self.shieldHPA = shield_hpa_coefficient * self.maxHPA
        self.power = 0
...
fighter.increase_max_power(True)  # Increase max power after low power attack
fighter.print_status()

print("High Power Attack Damage:", fighter.high_power_attack())
fighter.apply_shield(False)
fighter.increase_max_power(False)  # Increase max power after high power attack
fighter.print_status()
